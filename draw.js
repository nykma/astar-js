(function () {
  var rectGroup = [],     // 方格矩阵
    ...
    canvas = new fabric.Canvas('c'),    // 画布
    canvasWidth = canvas.getWidth(),
    canvasHeight = canvas.getHeight(),     // 画布尺寸
    startPoint = {X: 0, Y: 0, G: 0, father: undefined},
    endPoint = [{X: 5, Y: 5}], //起点和终点
    blockList = []; // 障碍物
  var canvasDrawGrid = function () {
    var lineGroup = [];
    var lineSpaceW = canvasWidth / mapW;
    var lineSpaceH = canvasHeight / mapH;
    for (var i = 1; i < mapW; i++) {
      var lineW = new fabric.Line([-(canvasWidth / 2), lineSpaceW * i, canvasWidth / 2, lineSpaceW * i], {
        stroke: 'orange',
        strokeWidth: 1,
        selectable: false
      });
    ...
    for (i = 1; i < mapH; i++) {
      var lineH = new fabric.Line([lineSpaceH * i, canvasHeight / 2, lineSpaceH * i, -(canvasHeight / 2)], {
        stroke: 'orange',
        strokeWidth: 2,
        selectable: false
      });
  ...
  var canvasDrawRect = function () {
    var W = canvasWidth / mapW;
    var H = canvasHeight / mapH;
